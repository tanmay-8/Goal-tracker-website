...
router.post(
  ...
  [
    ...
    body("password", "Enter password min length 5").isLength({
      min: 8,
      max: 18,
    }),
  ],
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user) {
        console.log("Sorry user with this email already exist.");
        return res
          .status(400)
          .json({
...
// ROUTE3:getting logined user details :GET"api/auth/getuser."  login rquired
